...
export const updatePost = (post:Post)=>{
    // postsState.update(prev=>({posts:[...prev.posts.filter(),post]}));
    // let postEdit = get(postsState).posts.find(item=>item.id === post.id);
    postsState.update(prev=>({posts:[...prev.posts.filter(item=>item.id !== post.id), post]}));
    localStorage.setItem('state', JSON.stringify(get(postsState)))
} 
